...
class Pairing_module {
	public:
		...
		Pairing_module(char *str){
			if(str){
				pbc_param_init_set_buf(p, str);
				pairing_init_pbc_param(pair_obj, p);
		...
		Element_class Element_elem(Pairing_module group, int type,unsigned long long_obj=LONG_MAX) {
